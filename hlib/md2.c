#include <string.h>
#include <hlib/md2.h>

const uint8_t hlib_md2_sbox[] = {
    ,
};

static void hlib_md2_transform_checksum(
    struct hlib_md2_context *ctx, const uint8_t *block)
{
    uint8_t L = ctx->checksum[15];

    for (int i = 0; i < 16; i++) {
        ctx->checksum[i] ^= hlib_md2_sbox[block[i] ^ L];
        L = ctx->checksum[i];
    }
}

static void hlib_md2_transform_state(
    struct hlib_md2_context *ctx, const uint8_t *block)
{
    uint8_t buf[32];
    uint8_t t = 0;

    for (int i = 0; i < 16; i++) {
        buf[i] = block[i];
        buf[i + 16] = buf[i] ^ ctx->state[i];
    }

    for (int i = 0; i < 18; i++) {
        for (int j = 0; j < 16; j++) {
            ctx->state[j] ^= hlib_md2_sbox[t];
            t = ctx->state[j];
        }

        for (int j = 0; j < 32; j++) {
            buf[j] ^= hlib_md2_sbox[t];
            t = buf[j];
        }

        t = (t + i) & 255;
    }
}

static void hlib_md2_transform(
    struct hlib_md2_context *ctx, const uint8_t *block)
{
    hlib_md2_tranform_checksum(block);
    hlib_md2_tranform_state(block);
}

void hlib_md2_init(struct hlib_md2_context *ctx)
{
    memset(ctx, 0, sizeof(*ctx));
}

void hlib_md2_update(struct hlib_md2_context *ctx, const void *src, size_t size)
{
    size_t block_size = HLIB_MD2_BLOCK_SIZE;
    size_t buffer_size = ctx->size & (block_size - 1);
    const uint8_t *data = src;

    ctx->size += size;

    if (buffer_size) {
        size_t buffer_rem = block_size - buffer_size;
        uint8_t *buffer_end = ctx->buffer + buffer_size;

        if (buffer_rem < size) {
            memmove(buffer_end, data, size);
            return;
        }

        memmove(buffer_end, data, buffer_rem);
        data += buffer_rem;
        size -= buffer_rem;
        hlib_md2_transform(ctx, ctx->buffer);
    }

    const uint8_t *data_end = data + size;
    const uint8_t *data_lim = data_end - block_size;

    for (; data <= data_lim; data += block_size) {
        hlib_md2_transform(ctx, data);
    }

    memmove(ctx->buffer, data, data_end - data);
}

void hlib_md2_finalize(struct hlib_md2_context *ctx, void *dest)
{
    size_t block_size = HLIB_MD2_BLOCK_SIZE;
    size_t buffer_size = ctx->size & (block_size - 1);
    size_t pad = block_size - buffer_size;

    memset(ctx->buffer + buffer_size, pad, pad);

    hlib_md2_transform(ctx, ctx->buffer);
    hlib_md2_transform_state(ctx, ctx->checksum);

    memmove(dest, ctx->state, sizeof(ctx->state));
}

void hlib_md2_hash(const void *src, size_t size, void *dest)
{
    struct hlib_md2_context ctx;
    hlib_md2_init(&ctx);
    hlib_md2_update(&ctx, src, size);
    hlib_md2_finalize(&ctx, dest);
}
